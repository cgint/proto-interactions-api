defmodule InteractionsPlayground.Live do
  ...
  defp extract_interaction_id(%{"interaction" => %{"id" => id}}) when is_binary(id) and id != "",
    do: id
  ...
  defp extract_status(%{"interaction" => %{"status" => status}})
       when is_binary(status) and status != "", do: status
